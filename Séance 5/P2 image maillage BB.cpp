...
Vecteur aleatoire(const Vecteur& N) { //Renvoie un vecteur z*N+x*T1+y*T2, avec (N,T1,T2) un repère, et x,y,z aléatoires suivant une loi de probabilité cosinus
...
public:
    ...
    virtual bool intersection(const Rayon& r, Vecteur& P, Vecteur& Normale, double& t) = 0;
    Vecteur albedo;
...
class BoundingBox {
public:
    bool intersect(const Rayon& r) { //intersection rayon boite
...
public:
    ...
    bool intersection(const Rayon& r, Vecteur& P, Vecteur& Normale, double& t) {
        if (!bb.intersect(r)) return false; //Si pas d'intersection avec la Bounding Box, pas d'intersection avec le maillage
        t = 1E9;
        bool intersecte = false;
        for (int i = 0; i < indices.size(); i++) { //pour tous les triangles du maillage
...
#pragma omp parallel for schedule(dynamic,1)
    ...
    stbi_write_png("image_plusrapideJeep.png", W, H, 3, &image[0], 0);
