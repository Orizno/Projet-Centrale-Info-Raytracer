...
int main() {
	...
	Vecteur up(0,cos(-anglecam*M_PI /180),sin(-anglecam*M_PI /180)); //verticale de la caméra
	Vecteur right(1,0,0); // droite
	Vecteur Direction=-prodVect(up,right); // direction de visée
...
#pragma omp parallel for schedule(dynamic,1)
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            ...
            for (int k = 0; k < nmbrayon; k++) {
                ...
                u=u[0] * right + u[1]*up + u[2]*Direction;
                u = u.normaliser();
    ...
    stbi_write_png("image_depcameraJeep.png", W, H, 3, &image[0], 0);
