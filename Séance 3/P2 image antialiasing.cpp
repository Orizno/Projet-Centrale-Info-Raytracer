...
Vecteur aleatoire(const Vecteur& N) { //Renvoie un vecteur z*N+x*T1+y*T2, avec (N,T1,T2) un repère, et x,y,z aléatoires suivant une loi de probabilité cosinus (le rayon a plus de chance d'être proche de N)
...
class Sphere { //classe pour les sphères (rajout d'un attribut pour savoir si c'est un miroir ou pas, et un autre pour la transparence)
public:
...
public:
    ...
    bool intersection(const Rayon& r, Vecteur& P, Vecteur& N, Vecteur& albedo, double& t, bool& miroir, bool& transp) {
        ...
        for (int i = 0; i < objets.size(); i++) {// pour chacune des sphères de la scène
            ...
            if (intersection(r,P,N,albedo,t,miroir,transp )) {// en cas d'intersection avec un objet de la scène
                if (miroir) { // si c'est un miroir
                    ...
                    if (transp) { // si  il est transparent
                        ...
                        Rayon Rayonombre(P + 0.001 * N, (L - P) / normePL); // Rayon partant du point d'intersection et dirigé vers la source de lumière
...
#pragma omp parallel for schedule(dynamic,1)
	...
	stbi_write_png("image_ecl_antialiasing.png", W, H, 3, &image[0], 0);
