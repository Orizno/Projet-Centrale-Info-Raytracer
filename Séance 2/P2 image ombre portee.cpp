...
class Sphere{ //classe pour les sphères
public:
...
int main() {
    ...
    double I =5E9; //intensité lumineuse
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
            ...
            if (scene.intersection(r,P,N,albedo,t )) {// en cas d'intersection avec un objet de la scène
                ...
                if (scene.intersection(Rayonombre, Pombre, Nombre, albedoombre, tombre) && tombre < normePL) { //s'il y a intersection avant d'arriver à la source de lumière
	...
	stbi_write_png("image_ombreportee.png", W, H, 3, &image[0], 0);
